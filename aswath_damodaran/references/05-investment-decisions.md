# Aswath Damodaran - Investment Decisions and Cases

## Overview

Unlike traditional value investors who publish their portfolios, Damodaran is known for his analytical approach to high-profile valuations rather than personal investment decisions. This document compiles his documented valuation cases and methodological applications.

---

## Famous Valuation Cases

### 1. Tesla (Multiple Times, 2010-2024)

**Timeline of Valuations:**

| Year | Valuation | Story |
|------|-----------|-------|
| 2010 | $20-30/share | Car company making electric cars |
| 2015 | $70-80/share | Leading EV company |
| 2018 | $220-250/share | Tech company with autonomous potential |
| 2020 | $350-500/share | Energy company + Auto |
| 2021 | Variable | High uncertainty |

**Methodology Used:**
- DCF with multiple scenarios
- Comparable company analysis
- Probability-weighted outcomes

**Key Quote (2021):**
> "I have valued Tesla more times than any other company. Each time, I learn something new. The valuation depends entirely on which story you believe about the future."

**The Tesla Framework:**

1. **Bull Case:** Tesla as tech company
   - Autonomous driving revenue
   - Energy business growth
   - Software margins
   - Valuation: $800+

2. **Base Case:** Tesla as car company
   - Auto sales growth
   - Margin compression
   - Competition increasing
   - Valuation: $200-400

3. **Bear Case:** Tesla as commodity auto
   - Price war
   - Margin destruction
   - Competition catching up
   - Valuation: <$100

---

### 2. Amazon

**Approach:**
Damodaran values Amazon differently depending on the business segments.

**Segments Analyzed:**
1. **E-commerce:** Low margin, high turnover
2. **AWS:** High margin, growing
3. **Prime:** Subscription moat
4. **Advertising:** Emerging profit center

**Key Quote:**
> "Amazon is either the most brilliantly run company in the world or the most creatively managed. The valuation depends on which story you believe."

**Methodology:**
- Sum-of-parts DCF
- Segment-level comparable analysis
- Probability-weighted scenarios

---

### 3. Bitcoin/Cryptocurrency

**Damodaran's Position:**
> "I have been asked to value Bitcoin. My answer is that Bitcoin has no intrinsic value, but that doesn't mean it isn't worth anything."

**Framework Applied:**
1. **Intrinsic Value:** None (no cash flows)
2. **Relative Value:** Compare to other stores of value
3. **Speculative Value:** Based on adoption curve

**Key Quote:**
> "Tulips had no intrinsic value either. But people paid fortunes for them. The question is not whether Bitcoin has intrinsic value. The question is whether you can predict the greater fool."

---

### 4. IPO Valuations

**General Approach:**

1. **Understand the Story:**
   - What is the company's growth narrative?
   - How sustainable is the competitive advantage?

2. **Build Multiple Scenarios:**
   - Base case
   - Bull case
   - Bear case

3. **Assign Probabilities:**
   - Based on historical success rates
   - Industry dynamics
   - Management track record

**Quote on IPOs:**
> "Most IPOs are priced to pop, not to reflect intrinsic value. If you want to invest in IPOs, you need to be prepared to sell quickly or have a long-term conviction."

---

## Valuation Methodologies Applied

### 1. DCF (Discounted Cash Flow)

**When Applied:**
- Stable, predictable businesses
- Companies with visible growth paths
- Long-term investment horizons

**Damodaran's DCF Components:**
1. **Revenue Growth:** Based on industry and market share assumptions
2. **Operating Margin:** Based on historical and peer data
3. **Cash Flow Conversion:** Based on business model
4. **Terminal Value:** Based on stable growth assumption
5. **Discount Rate:** Based on risk profile

### 2. Comparable Company Analysis

**When Applied:**
- IPO valuations
- M&A analysis
- Quick estimates

**Key Multiples Used:**
- EV/EBITDA
- P/E
- EV/Revenue
- P/B

**Damodaran's Approach:**
> "Comparables are only useful if you understand why a company trades at a multiple. The multiple tells a story about future expectations."

### 3. Probability-Weighted Scenarios

**When Applied:**
- High uncertainty companies
- IPOs
- Turnaround situations

**Framework:**
1. Build 3-5 scenarios
2. Estimate value in each scenario
3. Assign probabilities
4. Calculate probability-weighted value

**Example:**
- 30% probability of bull case: $100/share
- 50% probability of base case: $50/share
- 20% probability of bear case: $20/share
- Expected value: $55/share

---

## Key Investment Principles

### 1. Story-Driven Investing

> "The best investments are the ones where you can tell a coherent story about the future. If you can't tell a story, you are speculating."

### 2. Uncertainty Pricing

> "I don't try to eliminate uncertainty. I try to price it. If you are not being compensated for uncertainty, you should not be making the investment."

### 3. Margin of Safety

> "The margin of safety is not just about price. It is about being comfortable with your assumptions. If you need a large discount to feel comfortable, you probably don't understand the business."

### 4. Long-Term Focus

> "The market is a weighing machine in the long run. If you believe in the story, you need to be patient."

---

## Risk Assessment Framework

### 1. Business Risk

- Competitive dynamics
- Technology disruption
- Regulatory changes
- Management quality

### 2. Financial Risk

- Leverage
- Liquidity
- Cash flow generation

### 3. Valuation Risk

- Model assumptions
- Terminal value sensitivity
- Discount rate uncertainty

---

## Decision Process

### For Every Valuation:

1. **Understand the Business**
   - What does the company do?
   - How does it make money?
   - What is the competitive advantage?

2. **Develop the Story**
   - Where is the company going?
   - What are the key growth drivers?
   - What could go wrong?

3. **Build the Model**
   - DCF with scenarios
   - Comparable analysis
   - Probability weighting

4. **Check and Cross-Validate**
   - Do the numbers support the story?
   - What do comparables suggest?
   - What does the market think?

5. **Make the Investment Decision**
   - Is there a margin of safety?
   - Can you tolerate being wrong?
   - What is your time horizon?

---

## Famous Quotes on Investment Decisions

> "I don't try to predict the future. I try to understand the range of possible futures and price them appropriately."

> "The difference between a good investor and a great investor is the willingness to say 'I don't know.'"

> "If you want to be a contrarian, you have to be right when the market is wrong. If you are contrarian for the sake of being contrarian, you will lose money."

> "The best investments are the ones where you can tell a coherent story about the future."

> "Diversification is protection against ignorance. If you don't understand a business, diversify."

---

## Sources

1. Damodaran's Blog: Tesla valuation series
2. YouTube: Valuation Master Class
3. Podcast appearances: Acquired, Knowledge Project
4. Academic papers on valuation methodology

